...
import { QuizImage, QuizTitle, QuizAuthor, QuizPublishedIcon } from "./quizinfo";
      ...
      <div className="h-50 w-full p-6 flex flex-col grow">
        ...
        <QuizTitle>{quiz.title}</QuizTitle>
          <Action
            ...
            onClickDelete={() => {quiz.onClickDelete(quiz.id)}}
            onClickFavorite={() => {quiz.onClickFavorite(quiz.id)}}
            onClickRename={() => {quiz.onClickRename(quiz.id)}}
            onClickDuplicate={() => {quiz.onClickDuplicate(quiz.id)}}
            onClickShare={() => {quiz.onClickShare(quiz.id)}}
          />
          </div>
        <div className="grow">
